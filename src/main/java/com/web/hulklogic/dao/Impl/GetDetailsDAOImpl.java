...
public class GetDetailsDAOImpl implements GetDetailsDAO {
	...
	private static final String GET_ALL_JOB_APPLICANT_TOKENS = "select * from "
			+ "(select jobid,jobtags,jobtitle from job where jobid =?) a," + "(select applicantid,"
			+ "languages0,languages1,languages2,languages3,languages4,languages5,"
			+ "languages6,languages7,languages8,languages9  " + " from applicantdetails) b";
	...
	public CreateJob getJobDetailsByJobid(int jobid) {
		...
		try {
			createjob = (CreateJob) getJdbcTemplate.queryForObject(GET_JOBS_INFO_BY_JOBID, new Object[] { jobid },
	...
	public Applicants getApplicantsByApplicatId(int applicantid) {
		...
		try {
			applicants = (Applicants) getJdbcTemplate.queryForObject(GET_APPLICANTS_BY_APPLICANTID,
					new Object[] { applicantid }, new BeanPropertyRowMapper(Applicants.class));
	...
	public User getUserByUserId(int userid) {
		...
		User user = null;
		try {
			user = (User) getJdbcTemplate.queryForObject(GET_USER_INFO_BY_ID, new Object[] { userid },
	...
	public List<ParserText> getParserTextByApplicantId(int applicantid) {
		logger.info("applicantid :" + applicantid);
		List<ParserText> parsertext = getJdbcTemplate.query(GET_PARSER_TEXT_BY_APPLICANT_ID,
				new Object[] { applicantid }, new BeanPropertyRowMapper(ParserText.class));
		return parsertext;
	}
	...
	public List<ParserText> getParserTextByJobId(int jobid) {
		logger.info("jobid :" + jobid);
		List<ParserText> parsertext = getJdbcTemplate.query(GET_PARSER_TEXT_BY_JOB_ID, new Object[] { jobid },
				new BeanPropertyRowMapper(ParserText.class));
		return parsertext;
	}
	...
	public List<JobTitle> getJobTitleOfAutoComplete(String jobtitle) {
		...
		List<JobTitle> getAutoCompleteJobtitle = namedParameterJdbcTemplate.query(GET_ALL_JOBTITLE, pss,
				new BeanPropertyRowMapper(JobTitle.class));
	...
	public List<JobTitle> getLocationleOfAutoComplete(String location) {
		...
		MapSqlParameterSource pss = new MapSqlParameterSource();
		pss.addValue("pname", finalName);
		List<JobTitle> getAutoCompleteLocation = namedParameterJdbcTemplate.query(GET_ALL_LOCATIONS, pss,
				new BeanPropertyRowMapper(JobTitle.class));
	...
	public List<GetTokens> getJobAndApplicantTokens(int jobid) {
		...
		try {
			getListTokens = getJdbcTemplate.query(GET_ALL_JOB_APPLICANT_TOKENS, new GetTokensResultSetExtractor(),
					jobid);
		} catch (Exception e) {
			logger.error("Exception is: " + e);
		}
	...
	public List<GetEmployee> getEmployeedetails(String designation, String organization) {
		...
		try {
			getListTokens = (List<GetEmployee>) getJdbcTemplate.query(CHECK_EMPLOYEE,
					new GetTokensResultSetExtractors(), designation, organization);
		} catch (Exception e) {
			logger.error("Exception is: " + e);
		}
		return getListTokens;
