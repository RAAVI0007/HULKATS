...
public class BulkEmail {
	private static Logger logger = Logger.getLogger(BulkEmail.class);
	...
	public boolean sendMail(BulkEmailSend bulkemailsend, AutoSignature autoSignature) {
		...
		try {
			...
			InputStream input = null;
				...
				input = new FileInputStream("/opt/hulkats/propertiesfiles/" + bulkemailsend.getPropertyname());
			 ...
			 String	port = prop.getProperty("port");
					//	serverUrl = "https://ezbizz.in/hulkatsapp";
						// serverUrl1=https://ezbizz.in/applicantapp
						// serverUrl2=https://ezbizz.in/employeeapp
						//imageUrl = "https://ezbizz.in/Hulkats/images/HulkATS_logo.png";
			...
			try {
				...
				logger.info(bulkemailsend.getEmails().size());
				logger.info(bulkemailsend.getTemplatedesignid());
				for (int i = 0; i < bulkemailsend.getEmails().size(); i++) {
					bulkemailsend.getEmails().remove("on");

					mimemessage.addRecipient(Message.RecipientType.BCC,
							new InternetAddress(bulkemailsend.getEmails().get(i)));

				}
				mimemessage.setSubject(bulkemailsend.getSubject());
				logger.info(bulkemailsend.getTemplatedesignid());
				String msg = null;
				if (bulkemailsend.getTemplatedesignid() == 1) {
					...
					msg += "<b>" + bulkemailsend.getBody() + "</b><br>";
				...
				} else if (bulkemailsend.getTemplatedesignid() == 2) {
				...
				// get attachement
				String attachment[] = bulkemailsend.getTotalDocuments();
