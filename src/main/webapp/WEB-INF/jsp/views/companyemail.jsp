...
<body class="fixed-nav sticky-footer bg-light " id="page-top"
			...
			<div class="card card-register mx-auto mt-5">
				...
				<div class="card-body">
					...
					<form id="registration" name="registration" style="display: none;">
						...
						<div class="form-group">
							<label for="organization">Host Name</label> <input
								class="form-control" type="email" name="organization"
								id="host" placeholder="Enter hostname">
						</div>
						<div class="form-group">
							<label for="email">Port Number</label> <input class="form-control"
								type="text" name="port" id="port" placeholder="Enter port number">
						</div>

						
						<div class="form-group">
							<label for="addr1">From MailID</label> <input class="form-control"
								type="text" name="addr1" id="username" placeholder="Enter mailid"
								required />
						</div>
						<div class="form-group">
							<label for="location">Password</label>  <input class="form-control"
								type="password" name="addr1" id="password" placeholder="Enter Password"
								required />
						</div>
	...
	<script src="<c:url value='/resources/js/jquery.mCustomScrollbar.min.js' />"></script>
...
function checkAjaxCall(){
	  ...
	  $('#error1').text('');
	  $('#error').text('');
	  if($('#host').val()=='' || $('#host').val().trim()==''){
			$('#error').text('Host is required !');
			$('#error1').text('Host Name is required !');
			$('#host').attr('style', "border-radius: 5px; border:#FF0000 1px solid;");
			return;	
		}else{
			user.host =$('#host').val();		
		}
	  if($('#port').val()=='' || $('#port').val().trim()=='' ){
		$('#error').text('Port is required !');
		$('#error1').text('Port is required !');
		$('#port').attr('style', "border-radius: 5px; border:#FF0000 1px solid;");
		return;	
	}else{
		user.port =$('#port').val();		
	}
	  if($('#username').val()=='' || $('#username').val().trim()=='' ){
			...
			$('#error1').text('username is required !');
	 ...
	 user.organization =  $("#companylist option:selected").text();
    $('#error').text('');
    $('#error1').text('');

	var jsonfile={emailcreation:JSON.stringify(user)}; 
