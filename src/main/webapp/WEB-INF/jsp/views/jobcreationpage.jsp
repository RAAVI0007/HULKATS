			...
			<div class="card-body">
				...
				<form id="jobregistration" name="jobregistration">
					<label for="jobtitle"><strong>Job Title</strong></label> <input
						...
						id="companyname" placeholder="" required> <br>
					...
					<div class="form-group">
						<div class="form-row">
							...
							<div class="col-md-6">
								...
								<input class="form-control" type="text" name="jobrole"
									id="jobrole" placeholder="HTML5,CSS3" required> <br>
							</div>
						</div>
					</div>
					...
					<input type="text" id="videourl" class="form-control"
						placeholder="video url" required> <br>
					...
					<div id="upload">
						<div id="jobup">
							<label for="cvupload"><strong>Upload Job
								...
								id="response" type="hidden" class="form-control" required /> <span
	 ...
	 function getdata() {
  	...
  	function checkAjaxCall(){
			...
			if($('#jobtitle').val()=='' || $('#jobtitle').val().trim()==''){
				...
				$('#jobtitle').attr('style', "border-radius: 5px; border:#FF0000 1px solid;");
				return;	
			}else{
			...
			if($('#companyname').val()=='' || $('#companyname').val().trim()==''){
				...
				$('#companyname').attr('style', "border-radius: 5px; border:#FF0000 1px solid;");
				return;	
			}else{
...
if($('#state option:selected').text()=='Select State' || $('#state option:selected').text().trim()=='Select State'){
...
if($('#city option:selected').text()=='Select City' || $('#city option:selected').text().trim()=='Select City'){
	...
	return;	
			...
			if($('#salary').val()=='' || $('#salary').val().trim()==''){
				...
				$('#salary').attr('style', "border-radius: 5px; border:#FF0000 1px solid;");
				return;	
			}else{
			...
			if($( "#salarycriteria option:selected" ).text()=='Select'){
    			...
    			$('#salarycriteria').attr('style', "border-radius: 5px; border:#FF0000 1px solid;");
    			return;	
    		}else{
			...
			if($("iframe#udescription2").contents().find('body').html()=='' || $("iframe#udescription2").contents().find('body').html()==''){
				$('#error').text('Job Description is required !');
				$('#error1').text('Job Description is required !');
				$("iframe#udescription2").contents().find('body').html().attr('style', "border-radius: 5px; border:#FF0000 1px solid;");
				return;	
			}else{
				var content=$("iframe#udescription2").contents().find('body').html();
				
				joblist.jobdescription =content;		
			}
			if($("iframe#udescription").contents().find('body').html()=='' || $("iframe#udescription").contents().find('body').html()==''){
				$('#error').text('Job Requirement is required !');
				$('#error1').text('Job Requirement is required !');
				$("iframe#udescription").contents().find('body').html().attr('style', "border-radius: 5px; border:#FF0000 1px solid;");
				return;	
			}else{
				var content=$("iframe#udescription").contents().find('body').html();
       	...
       	var jsonfile={jobcreation:JSON.stringify(joblist)}; 
	...
	return false;	
		 ...
		 function updatejob(){
			 ...
			 var joblist ={};
				...
				if($('#location option:selected').text()=='Select City and State' || $('#location option:selected').text().trim()=='Select City and State'){
					$('#error').text('City & State is required !');
					$('#error1').text('City & State is required !');
					
					$('#location').attr('style', "border-radius: 5px; border:#FF0000 1px solid;");
					return;	
				}else{
					joblist.location =$('#location option:selected').text();		
				}
	...
	function firstdocdel(){
		...
		.ajax({
			...
			data : {jobId:jobid,filename:doc1,fileNumber:1},
			dataType : "json",
			complete : function(data) {
				console.log(data);
				documents(jobid);
	...
	function seconddocdel(){
		...
		.ajax({
			type : "POST",
			url : "${pageContext.servletContext.contextPath}/user/deleteJobFile",
			data : {jobId:jobid,filename:doc2,fileNumber:2},
			dataType : "json",
			complete : function(data) {
				console.log(data);
				documents(jobid);
